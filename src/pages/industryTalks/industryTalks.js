...
function IndustryTalks() {
    ...
    useEffect(() => {
        ...
        function moveTooltip(e, index) {
            ...
            if (actualTopLeftCorner.top > 350 + tooltipDimensions.height) { //* Additional conditions - ((actualTopLeftCorner.left > tooltipDimensions.width * 0.001 && ... ) && (actualTopLeftCorner.left < 800 - tooltipDimensions.width && actualTopLeftCorner.top > tooltipDimensions.height * 0.001))
                //* but causes stuck rendering
                tooltips[index].style.top = actualTopLeftCorner.top + 'px';
            }
            if (actualTopLeftCorner.left > 240 - tooltipDimensions.width * 0.5 && actualTopLeftCorner.left < 240 + tooltipDimensions.width * 0.5) {
                ...
                <>
                    <div className="talk_card">
                        ...
                        <div className="talk_datetime">
                            <h1>{talkData.date.slice(0, 2)}<sup>th</sup></h1>
                            <time>{talkData.time}</time>
                            <address>📍 {talkData.place}</address>
                        </div>
                        <div className="card_back" style={{ "backgroundImage": `url(${importedCoverImages[talkData.coverImage]})` }}></div>
