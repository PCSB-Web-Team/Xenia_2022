...
const handleVerifyClick = (eventId, userId) => {
  Requests.verifyParticipant({
    userId: userId,
    eventId: eventId,
  });
};
...
const participantsSchema = [
  ...
  },
  {
    ...
    cell: (row) =>
      row.isVerified ? (
        <div className="text-white bg-gradient-to-r from-green-500 to-green-500 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium  px-5 py-2.5 text-center text-lg">
          Verified
        </div>
      ) : (
        <button
          ...
          onClick={() => {
            ...
            handleVerifyClick(row.eventId, row.userId);
          }}
        >
          Verify
        </button>
...
export default function Admin(props) {
  ...
  useEffect(() => {
    console.log(participantsTableData);
    ...
    try {
      await axios
        ...
        .then(async (res) => {
          if (res?.data?.status) {
            setLoading(false);
            let userTableData = await res?.data?.data?.data.map(
              ({ name, mobile, email, createdAt }, index) => {
                ...
                return {
                  ...
                  mobile,
                  email,
                  createdAt: parsed_createdAt.toUTCString(),
                };
              }
            );
  ...
  async function fetchAllParticipants() {
    ...
    try {
      await axios
        .get(process.env.REACT_APP_API_URL + "/participant")
        .then(async (res) => {
          if (res?.data?.status) {
            setLoading(false);
            let participantsTableData = await res?.data?.data?.data.map(
              (
                {
                  email,
                  mobile,
        ...
        .catch((error) => {
          props.toast.toast.error(
            "Error while fetching participants data! : " + error?.message
          );
        });
    } catch (error) {
      setLoading(false);
    ...
    try {
      await axios
        ...
        .then(async (res) => {
          if (res?.data?.status) {
            setLoading(false);
            let teamsTableData = await res?.data?.data?.map(
              ({ count, eventId, isFull, max, name, _id }, index) => ({
                id: index + 1,
                name,
                _id,
                eventId,
        ...
        .catch((error) => {
          props.toast.toast.error(
            "Error while fetching teams data! : " + error?.message
          );
        });
    } catch (error) {
      setLoading(false);
  ...
  async function fetchEventsData() {
    ...
    await Requests.getEvents()
      ...
      .catch((err) => {
        setLoading(false);
        props.toast.toast.error(
          "Error while fetching events data! : " + err?.message
        );
      });
    setLoading(false);
  }
  ...
  useEffect(() => {
    !auth.isAdmin &&
      props.toast.toast.warn(
        "Please authenticate as Admin before accessing Admin Page!"
      );
    ...
    <div className="admin-page h-screen">
      <div className="admin-login">
        ...
        <input
          ...
          onChange={(e) =>
            setAuth((prevAuth) => ({ ...prevAuth, username: e.target.value }))
          }
          required
        />
        <input
          ...
          onChange={(e) =>
            setAuth((prevAuth) => ({ ...prevAuth, password: e.target.value }))
          }
          required
        />
        <button type="submit" onClick={async (e) => await authAdmin(e)}>
          Login
        </button>
      </div>
    </div>
        ...
        <span>
          <button
            ...
            className={`p-2 rounded mx-4 ${
              services === 1 ? `bg-white text-black` : `bg-black text-white`
            }`}
          >
            Check Database
          </button>
          <button
            ...
            className={`p-2 rounded mx-4 ${
              services === 2 ? `bg-white text-black` : `bg-black text-white`
            }`}
          >
