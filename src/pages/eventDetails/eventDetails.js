...
const EventDetails = (props) => {
  ...
  const handleRegisterEvent = async () => {
    setLoading(true);
    let { data } = await Request.registerEvent({
      eventId: id,
      transactionID: 0,
    });
    if (data?.status) {
  ...
  async function fetchEventData() {
    ...
    try {
      ...
      props.toast.toast.error(
        "Error: server unreachable, at the moment. " + error || ""
      );
      navigate("404");
    }
  }
  ...
  async function fetchIsUserParticipated() {
    try {
      ...
      props.toast.toast.error(
        "Error: couldn't fetch participation information." + error || ""
      );
    }
  }
  ...
  useEffect(() => {
    if (!userState.loggedIn) {
      ...
      props.toast.toast.warn(
        "You are not logged in! Login or try refreshing the page."
      );
    }
                  ...
                  <ol className="text-gray-300 font-thin  list-disc list-inside">
                    {eventData?.schedule?.map((data) => (
                      <li>
                        Round {data.round} :{" "}
                        {data.datetime !== "" ? (
                          <a href="https://www.pcsbxenia.com/schedule">
                            {data.datetime}
                          </a>
                        ) : (
                          <a href="https://www.pcsbxenia.com/schedule">
