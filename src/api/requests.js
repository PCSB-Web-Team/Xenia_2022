...
backend.defaults.headers.common["authorization"] =
  "Bearer " + localStorage.getItem(process.env.REACT_APP_TOKEN_NAME);
...
const verifyParticipant = async (data) =>
  await backend.post("/participant/verify", data);

const registerEvent = async (event) =>
  await backend.post("/participant", event);

const createTeam = async (data) =>
  await backend.post("/participant/team", data);
...
const sendContactUsMessage = async (message) =>
  await backend.post(
    "https://xenia-mailer.herokuapp.com/api/contact-us",
    message
  );
